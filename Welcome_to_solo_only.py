...
их получит бот''')) 
    ...
    # когда в колоде остается меньше 3 карт - т.е. нельзя сформировать выбор из 3 карт - колода замешивается заново из сброса
    if not cards:
    ...
    # опция для завершения игры и подсчета очков
